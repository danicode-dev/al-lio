...
test("every password/session entry point still lands on /dashboard and lets the gate decide (issue #290)", async () => {
  const [passwordLogin, passwordReset, resetForm, confirmRoute, googleCallback] = await Promise.all([
    readSource("src/lib/auth/password-login.ts"),
    readSource("src/lib/auth/password-reset.ts"),
    readSource("src/components/auth/reset-password-form.tsx"),
  ...
  // A completed reset signs the user in and routes them to /dashboard from
  // its confirmation panel instead of an abrupt redirect (issue #272); the
  // destination - and therefore the gate - is unchanged.
  assert.match(passwordReset, /return \{ error: null, ok: true \};/);
  assert.match(resetForm, /if \(state\.ok\) \{/);
  assert.match(resetForm, /<Link href="\/dashboard">/);
