...
```http
X-AL-LIO-Schema-Version: 2
X-AL-LIO-Delivery-Id: <uuid>
X-AL-LIO-Timestamp: <ISO-8601 UTC timestamp>
X-AL-LIO-Signature: sha256=<HMAC-SHA256 hex digest>
```
...
queries join the authenticated user's profile and require the profile cycle to
be present in `target_cycle_codes`. Expired items are excluded. Read and saved
state is owned per user in `radar_item_user_states`.
