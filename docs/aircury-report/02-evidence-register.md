...
| `DEL-008` | delivered | AL-LIO had not previously been submitted to another contest or grant programme | Owner statement recorded in issue #315 on 31 August 2026 | public | verified by owner statement |
...
|---|---|---|---|---|---|---|
| `PRD-001` | Public project entry and public information | Canonical public and legal routes only | Issue #297 implementation/test map and owner smoke test on 31 August 2026 | delivered | public | verified |
| `PRD-002` | Account creation, confirmation, login, reset, Google identity and signed access | Implemented account flows; external-provider checks remain separate | Issue #297 implementation/test map and owner smoke test on 31 August 2026 | delivered | public | verified for registration, login, logout, reset and Google identity |
| `PRD-003` | Onboarding, profile updates and product tour | Authenticated profile and tour flow | Issue #297 implementation/test map and owner smoke test on 31 August 2026 | delivered | public | verified |
...
| `PRD-005` | Dashboard and Quick Add | Authenticated dashboard boundary | Issue #297 implementation/test map and owner smoke test on 31 August 2026 | delivered | public | dashboard verified; Quick Add not separately recorded |
| `PRD-006` | Competency roadmap and private cycle progress | Authenticated cycle-scoped route | Issue #297 implementation/test map and owner smoke test on 31 August 2026 | delivered | public | route/progress verified; final per-cycle coverage pending |
| `PRD-007` | Approved learning resources, player progress and notes | Approved/available exact-cycle resources and private state | Issue #297 implementation/test map and owner smoke test on 31 August 2026 | delivered | public | progress and notes verified; final resource coverage pending |
| `PRD-008` | Private task lifecycle | Current user's task rows only | Issue #297 implementation/test map and owner smoke test on 31 August 2026 | delivered | public | create, update and completion verified |
| `PRD-009` | Private Bloc notes, recovery and export | Current user's note rows only | Issue #297 implementation/test map and owner smoke test on 31 August 2026 | delivered | public | note persistence verified; recovery/export not separately smoked |
| `PRD-010` | Local and optional Google Calendar flows | Local planning plus separately consented provider flow | Issue #297 implementation/test map; `SEC-005`; owner smoke test on 31 August 2026 | implemented | public | Google identity works; Calendar consent blocked by Google's unverified-app warning, so connect/disconnect was not completed |
| `PRD-011` | Authorised cycle-news list, detail, read and save boundary | Approved, fresh, non-withdrawn exact-cycle news | Issue #297 implementation/test map; owner smoke test; rollout issue #235 | implemented | public | production smoke failed or returned little/no content; final aggregate unavailable |
...
| `PRD-017` | Shared desktop/mobile navigation hierarchy | Equivalent core route hierarchy, not pixel identity | Issue #297 implementation/test map; owner statement on 31 August 2026 | internally validated | public | desktop Chrome verified; responsive/mobile navigation reported working, but no identified mobile device/browser session was recorded |
| `PRD-018` | Verified end-to-end student journeys in the final release | Only journeys passing the final acceptance matrix | Issue #297, tests and `QAL-002` | measured | aggregate-only | partially verified; Calendar and news prevent a complete pass |
...
| `SEC-005` | Google identity and Calendar are separate; Calendar credentials are owner-bound and every Calendar route validates the AL-LIO session | Optional integration boundary | PR #312; `tests/integration/auth/calendar-ownership.test.mjs`; issue #298 | implemented | public | CI-verified; production Calendar consent blocked by Google's unverified-app warning |
...
| `ENG-003` | Automated test files and final test result | 41 tracked `*.test.mjs` files; final CI runner summary 345 passed, 0 failed | Tag `aircury-2026-delivery`; CI run `33404234578` | measured | public | verified |
| `ENG-004` | Applied schema migrations represented by the release | 15 ordered migration files (`0002`–`0016`) in the tag; deployment reported no new pending migration for the release | Tag inventory; deployment run `33404461730` | measured | public | tag inventory verified; separate current production status not exported publicly |
...
| `QAL-001` | The delivery release passes the repository CI contract | GitHub Actions CI on exact SHA `1e516ead8f69d60a263718c20d59b97c9618c97a`; run `33404234578` | Issue #295 and issue #299 | measured | public | verified |
| `QAL-002` | Final owner smoke test covers the principal production flows | Owner statement dated 31 August 2026; desktop Chrome; no screenshots or personal data retained | Issues #299 and #315 | measured | aggregate-only | partial: core flows pass; Calendar blocked; news fails/is sparse; no identified mobile-device run |
...
| `OPS-004` | Backup, isolated restore and rollback readiness | Owner reports a local off-VPS copy; schedule, retention, encryption and last isolated restore are not evidenced | Issues #299, #315 and #317 | measured | aggregate-only | incomplete; automation and recovery evidence remain open |
...
| `IMP-001` | Student task completion and perceived relevance | Consented study with sample size, scenarios and limitations | Issue #300 and owner statement in #315 | measured | aggregate-only | not conducted; future study planned, no measured user impact claimed |
...
| `IMP-003` | Mobile, keyboard, focus, reduced-motion and semantic mechanisms exist without a WCAG-conformance claim | Reviewed shared UI and structural tests | Issues #300 and #315 | internally validated | public | mechanisms collected; no complete manual or independent accessibility audit |
...
| `ECO-001` | Current and projected monthly operating cost through 31 August 2027 | Owner-stated EUR costs: VPS EUR 28/month, domain EUR 15/year, transactional email EUR 0 at current usage; monitoring and off-host backup costs excluded until selected | Issues #300 and #315 | estimated | aggregate-only | estimated known base: EUR 29.25/month equivalent and EUR 351 for twelve months; VAT/allocation not confirmed |
| `ECO-002` | Maintenance responsibilities and provider fallbacks support continued operation | OVH hosting and Cloudflare registrar/DNS; owner responsible for incidents, releases, security and Radar editorial review | Issues #299, #300, #315, #316 and #317 | implemented | public | ownership confirmed; monitoring and backup controls remain incomplete |
