...
| `docs/audits/route-budgets.json` | `retained evidence` | Critical-route performance issue #331 under parent #361 | Consumed by `scripts/check-route-budgets.mjs` and the CI `perf:budgets` gate | None; this baseline has no publication authority | Reproduce with `npm run build` then `npm run perf:budgets`; method in `docs/operations/PERFORMANCE_BASELINE.md` | Public route names and gzipped bundle sizes; no database records, secrets or user data | Reviewed first-load-JS baseline that fails CI on a per-route bundle regression above its budget | Remove only when a replacement bundle-regression control is merged with equivalent per-route budgets |
