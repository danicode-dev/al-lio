...
export function parseDatasetSource(sourceText, sourcePath) {
  ...
  if (Array.isArray(parsed.companies)) return { cycleGroupInFile: parsed.cycleGroup ?? null, rows: parsed.companies };
...
export function validateDataset({ cycleGroupInFile, rows, cycleGroup, existingIdentities = new Map() }) {
  const errors = [];
  const warnings = [];
  ...
  rows.forEach((raw, index) => {
    ...
    if (!fuente) errors.push(`${label}: fuente is required`);
